...
$(document).ready(function () {
    ...
    $('input.layerBtn').on('change', function (event) {
        var spanText = $(this).closest('li').find('span')[0].innerText;        
        _addKMLLayerToMap(spanText, $(this)[0].checked);
    });
    
    $('input.statusBtn').on('change', function (event) {
        var spanText = $(this).closest('li').find('span')[0].innerText;
        _showImprovementStatusData(spanText, $(this)[0].checked);
    });
    ...
    function _initSelection() {
	 ...
	 setTimeout(function () {
        ...
        $('#BADEZoneChk').iCheck("check");
        $('#BADEVillageChk').iCheck("check");
    }, 800);
});
...
function _showProfile(inspectionId) {
    $.ajax({
        type: "POST",
        url: "/TechximBADE/Inspection/ShowProfile",
...
function _search() {
    ...
    $.ajax({
        type: "POST",
        url: "/TechximBADE/Inspection/FindInspectionData",
...
function _showPieChart(data) {
    ...
    $.ajax({
        type: "POST",
        url: "/TechximBADE/Inspection/FindPieChartData",
        data: $("#conditionForm").serialize()
...
function _deleteAllSelected() {
    if (confirm('是否刪除所勾選的資料?')) {
        $.ajax({
            type: "POST",
            url: "/TechximBADE/Inspection/DeleteAllInspectionIds",
...
function _downloadOneWord(inspectionid) {
    var form = $('<form></form>');
    $(form).hide().attr('method', 'post').attr('action', "/TechximBADE/Inspection/DownloadWord");
...
function _downloadOnePDF(inspectionid) {
    var form = $('<form></form>');
    $(form).hide().attr('method', 'post').attr('action', "/TechximBADE/Inspection/DownloadPDF");
    $(form).attr('target', '_blank');
    var input = $('<input type="hidden" name=inspectionId value="' + inspectionid + '"/>');
    $(form).append(input);
    $(form).appendTo('body').submit();
...
function _downloadFile(inspectionid) {
    var form = $('<form></form>');
    $(form).hide().attr('method', 'post').attr('action', "/TechximBADE/Inspection/DownloadFile");
    $(form).attr('target', '_blank');
    var input = $('<input type="hidden" name=inspectionId value="' + inspectionid + '"/>');
    $(form).append(input);
    $(form).appendTo('body').submit();
...
function _downloadExcel() {
    ...
    var form = $('<form></form>');
    $(form).hide().attr('method', 'post').attr('action', "/TechximBADE/Inspection/DownloadExcelByCondition");
    $(form).attr('target', '_blank');
...
function _downloadPDF() {
    ...
    $.ajax({
        type: "POST",
        url: "/TechximBADE/Inspection/BatchTransForm",
        data: condition
    }).done(function (data) {
        inspectionData = data;
        _initGrid(data);
